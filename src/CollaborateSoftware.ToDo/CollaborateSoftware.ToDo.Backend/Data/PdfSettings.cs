using System;
using System.Collections.Generic;

namespace CollaborateSoftware.MyLittleHelpers.Backend.Data
{
    public class PdfSettings
    {
        public string Type { get; set; }
        public bool IncludeAppointments { get; set; }
        public bool IncludeFincance { get; set; }
        public string Year { get; set; }
        public DateTime Date { get; set; }
        public string Note { get; set; }
        public string ForTomorrow { get; set; }
        public string Priority1 { get; set; }
        public string Priority2 { get; set; }
        public string Priority3 { get; set; }
        public string Priority4 { get; set; }

        public string ActionStep1 { get; set; }
        public string ActionStep2 { get; set; }
        public string ActionStep3 { get; set; }
        public string ActionStep4 { get; set; }
        public string ActionStep5 { get; set; }
        public string ActionStep6 { get; set; }
        public string ActionStep7 { get; set; }
        public string ActionStep8 { get; set; }
        public string ActionStep9 { get; set; }
        public string ActionStep10 { get; set; }
    }

}
